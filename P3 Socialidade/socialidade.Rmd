...
`dados`. Além dos dados oficialmente publicados no suplemento, após
pedido por e-mail os autores nos cederam também dados adicionais.
Confira as planilhas disponíveis no arquivo mencionado, usando a
planilha `metadados` para entender seu contexto.

Ok, agora importe os dados para dentro de um **objeto** **de R**. Vamos
começar usando apenas a **planilha** que contém os dados, dentro do
arquivo XLSX de origem. Depois você pode conferir a outra planilha, com
os metadados.

Vamos garantir que as **datas** estão no formato correto e que apenas os
**eventos efetivos** de compartilhamento de sangue, que duraram mais do
...
morcegos-vampiros de uma mesma colônia mantida em cativeiro. Que tal
então aprender um pouco sobre **ciência de redes** nesta prática?
...
conexões eram as relações de **compartilhamento** **de sangue**, seu
único alimento, entre esses indivíduos mantidos em cativeiro.
...
regurgitado, e `receiver`, o morcego individual que recebeu a doação de
alimento. Notem quem é quem, em cada caso:
...
Como havia repetição desses nomes no objeto, já que mais de um evento de
doação foi registrado para cada dupla de donor-receiver, precisamos
**resumir os dados** pela frequência dos eventos:
...
Crie um novo objeto com as **propriedades dos nós**, unindo as
informações dos objetos anteriores:
...
Adicione à rede as **informações** sobre sexo e idade:
...
frequência de eventos de doação entre os pares de morcegos:
...
Antes de plotar o grafo, ou seja, o desenho que representa a rede,
defina a **espessura** dos elos como sendo os valores normalizados das
frequências:
...
Defina as **cores** dos elos, nós e rótulos:
...
**Elimine** os nós que não tenham todas as informações sobre idade:

```{r}
rede2 <- delete_vertices(rede2, V(rede2)[is.na(V(rede2)$age)])
```

Defina o **layout** do grafo, ou seja, seu algoritmo de desenho:
