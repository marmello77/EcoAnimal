...
```{r}
rede1 <- graph_from_data_frame(dados1[, c("keydonor", "subject")], directed = FALSE)
```
...
Que tal dar um pouco de cor à rede? Os nós, elos e rótulos receberão
cores diferentes:

```{r}
V(rede1)$color <- "blue"      # Nós azuis
E(rede1)$color <- "red"       # Elos vermelhos
V(rede1)$label.color <- "white" # Rótulos brancos
```

Plote a rede para ver como ela ficou:
...
num_novos_elos <- round(0.5 * gsize(rede1))  # Adicionar 50% de elos extras

novos_elos <- data.frame(
  keydonor = sample(V(rede1)$name, num_novos_elos, replace = TRUE),
  subject  = sample(V(rede1)$name, num_novos_elos, replace = TRUE)
)

novos_elos <- novos_elos[novos_elos$keydonor != novos_elos$subject, ]
novos_elos <- novos_elos[!duplicated(novos_elos), ]

rede1 <- add_edges(rede1, t(as.matrix(novos_elos)))

E(rede1)$color[(gsize(rede1) - nrow(novos_elos) + 1):gsize(rede1)] <- "orange"
