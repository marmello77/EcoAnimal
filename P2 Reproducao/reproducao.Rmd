...
Quais variáveis operacionais foram definidas para análise no estudo?
...
umas às outras? Ou seja, quais foram as previsões testadas no estudo?
...
Felzimente, os autores do artigo de base incluíram um suplemento, com os
...
Localize-se no arquivo `dados.xlsx`.
...
Olhe os valores do eixo Y. Tem algo diferente?

Transforme a variável Y, da mesma maneira como fizeram no artigo de
base:
...
que eles provavelmente usaram nesta análise:
...
```{r}
table(dados$Duration.of.thanatosis.bin)
```

Plote um gráfico igual ao da Figura 2b:
