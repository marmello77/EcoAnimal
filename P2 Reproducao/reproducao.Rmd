...
Remova as linhas com valores **ausentes** (NA):

```{r}
dados1 <- dados1 %>% filter(!is.na(LEN) & !is.na(TAIL) & !is.na(TUSK))
```

Garanta que os dados principais a serem usados nas análises são
entedidos como **numéricos**:
...
Remova dados outliers sem explicação:
...
plot <- ggplot(dados2, aes(x = LEN, y = Valor, color = Medida)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_smooth(data = subset(dados2, Medida == "TAIL"), method = "lm", formula = y ~ x, se = FALSE) +
  geom_smooth(data = subset(dados2, Medida == "TUSK"), method = "lm", formula = y ~ poly(x, 2), se = FALSE) + 
...
Você acha que hipótese de trabalho foi corroborada ou não? Por que?
